...
exports.select10Estate = function(index, callback){
    connection.query(
        "select name, area, street_name, city, price from estate inner join location on location.id = estate.id order by date desc, cur_time desc limit ?, ?", [index*10, (index + 1)*10], function(err, res, fields){
            if(res !== undefined){
